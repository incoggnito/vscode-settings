...
from app.pages2.calendar import load_cal_by_sql, string2date_tz
...
def workinghours_view(
    ...
    wtimes = df[["worktime_dec", "daily_worktime"]].dropna().round(3)
        ...
        if df.empty:
            ...
            if (not get(f"monthly/{s.year}/{s.month}")["month"] == s.month) or (
                ...
                if st.button("Speichern"):
                    ...
                    last_valid_meta = get(f"meta/{s.year}/{s.month}")
                    del last_valid_meta["submitter"]
                    del last_valid_meta["department"]
                    del last_valid_meta["date_created"]
                    last_valid_meta["year"] = s.year
                    last_valid_meta["month"] = s.month
                    post("meta", data=last_valid_meta)
