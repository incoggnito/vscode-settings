...
try: 
    __import__(win32com)
    import outlook_mail.EmailHandler
    ...
    def test_catch_no_outlook(mocker: MockerFixture) -> None:
        ...
        """
        mock_dispatch = mocker.patch("outllok_mail.EmailHandler.mail.win32.Dispatch")
        mock_dispatch.side_effect = com_error("Mocking stimulated win32com.client.Dispatch com_error")
        with pytest.raises(SystemExit):
            outllok_mail.EmailHandler.Mail()
    ...
    def test_call_send_with_right_params(mocker: MockerFixture) -> None:
        ...
        Args:
            mocker (MockerFixture): pytest MockerFixture
        """
        mocker.patch("outllok_mail.EmailHandler.mail.win32.Dispatch")
        testmail = outllok_mail.EmailHandler.Mail()
    ...
    LOGGER.info("On a linux machine. EmailHandler only available for Windows machines.")
except ImportError:
    LOGGER.info("ImportError: On a linux machine. EmailHandler only available for Windows machines.")