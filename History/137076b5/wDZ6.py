...
class Ui_FRF_Analyzer(object):
    def __init__(self, df):
        ...
        self.peaks, self.properties = find_peaks(self.feature, prominence=0.2, width=1, distance=10)
    ...
    def get_all_peaks(self):
        ...
        # find peaks
        self.x_peak, _ = find_peaks(
            self.feature,
            height=self.border,
            prominence=[
                0.01 * np.max(self.abs_TF[self.xu : self.xo_peak]),
                np.max(self.abs_TF[self.xu : self.xo_peak]),
            ],
        )
        if len(self.x_peak) == 0:
            self.x_peak, _ = find_peaks(
                self.abs_TF[self.xu : self.xo],
                distance=10,
                prominence=[
                    0.005 * np.max(self.abs_TF[self.xu : self.xo_peak]),
                    np.max(self.abs_TF[self.xu : self.xo_peak]),
                ],
            )
    ...
    def add_list_items(self):
        ...
        """
        items = [f"trai {self.peaks[i]}" for i in range(len(self.peaks))]
    ...
    def PeakRegionUpdated(self):
        ...
        self.z, self.nf, self.err = self.sdof_cf(
            self.f_column, self.TF_column, self.unter_index, self.ober_index, plot_it=True
        )
        damping = 100 * self.z
    ...
    def PeakRegionUpdatedFinished(self):
        ...
        self.list_peaks.item(rownum).setText(
            str(self.eigenfrequenz[rownum]) + "Hz-err:" + str(round(sqrt(self.err) / self.max_residuum, 3))
        )
    ...
    def setupUi(self, FRF_Analyzer):
        ...
        self.main_view.plot(self.feature.values)
        # peaks mit roten Kreuzungen markiert
        # self.peak_mark=self.main_view.plot(self.peaks, self.feature, pen = None, symbol='x',symbolPen = ('r'))

        # self.c2=self.peak_view.plot(self.x[self.xu:self.xo], self.feature[self.xu:self.xo])
