...
class Ui_FRF_Analyzer(object):
    ...
    def __init__(self, filename:str):
        ...
        #get the parameters of the peaks
        if self.calc_peaks == True:
            #print('Find Peaks')
            self.get_all_peaks()
            #print('finished.\n')

        self.f_column = self.f.reshape((self.lenf,1))
        self.TF_column = self.TF.reshape((self.lenf,1))
        #print('SDOF Fit')
        self.sdof_fit()
        #print('finished.\n')
    ...
    def get_all_peaks(self):
        ...
        #border coherence > 0.9 and rolling mean value
        self.border=np.zeros(len(self.coh[self.xu:self.xo_peak]))
        self.border[np.argwhere([self.coh[self.xu:self.xo_peak] < 0.9])[:,1]] = np.max(self.abs_TF[self.xu:self.xo_peak])

        self.border = self.border+np.convolve(self.abs_TF[self.xu:self.xo_peak], np.ones((1000,))/1000, mode='same')
