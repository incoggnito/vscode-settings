    ...
    for fname, fileobj in folder.files.items():
        if fname.endswith("pp.trfdb"):
            ...
            with fileobj() as trfsb:
                df = trfsb.read()
                df["fileid"]=fname.split("_")[0]
                df_List.append(trfsb.read())

    df = pd.concat(df_List)
    df = df.drop(["Channel", "Time rel. [s]"], axis=1)
...
def merge_new_to_old_features_on_time(old: pd.DataFrame, new: pd.DataFrame) -> pd.DataFrame:
    new["Time"] = pd.to_datetime(new["Time_1"])
...
if __name__ == "__main__":
    wkd = "C:/Users/ahofer/Desktop/SmartSAD/Durchlauf_9_05_auf_8_3"
    create_trfdb(wkd)
