...
def merge_new_to_old_features_on_index(old: pd.DataFrame, new: pd.DataFrame) -> pd.DataFrame:
    ...
    old = old.set_index(old["trai_ch2"], drop=True)
    return old.join(new, how="left")
...
if __name__ == "__main__":
    ...
    df3 = merge_new_to_old_features_on_time(old_features ,new_features)
    # TODO sort
    df2 = df.drop(["trai_ch2"], axis=1)
    conn = sqlite3.connect(f"{wkd}/master.db")
    df2.to_sql("measurement", conn, if_exists="replace")

    conn = sqlite3.connect(f"{wkd}/master_time.db")
    df3.to_sql("measurement", conn, if_exists="replace")