...
Die Zusta
Die Signale, die ein Sensor misst, werden bei Bedarf verstärkt und sind die Eingangsgröße des VAM zur Berechnung der Features. Anhand der Features wird der aktuelle Zustand des Ziehprozesses bewertet und zu den Zustandsdaten weitergeleitet. Zusätzlich fließen alle Daten des Betriebs wie unter anderem die Ziehgeschwindigkeit, der Ausgangsdurchmesser des Drahtes und Informationen des Ziehsteins in die Zustandsdaten ein. Diese Zustandsdaten werden mit einer GUI dargestellt und können in Firmendatenbanken abgespeichert werden.
...
(AH) Bitte auf Darstellbarkeit in Latex achten!
:::
...
$\sigma(x)*i = \frac{e^{x_i}}{\sum^{K}*{j=1}e^{x_j}}$
