...
if __name__ == "__main__":
    ...
    from sqldb import DbFile
    
    # f = FileHandler("feature_data.pkl", subdir=["data"])
    f = FileHandler("test_database.db", subdir=["data"])
    df = pd.DataFrame(f.read("measurement"))

    # remove constants
    df = df.loc[:, (df != df.iloc[0]).any()]

    # Reset index
    #df.set_index("trai_ch1", inplace=True)

    # remove enumerations
    df2 = df.get_numeric_data()
    df3 = df2.diff().round(3).iloc[1:-1]
    df2 = df2.loc[:, (df3 != df3.iloc[1]).any()]
