...
async def create_meta(
    meta_in: MetaCreateFull,
    db: Session = Depends(deps.get_db),
    uid: int = Depends(deps.get_current_user),
) -> Meta:
    """_summary_
    ...
    Args:
        ...
        uid (int, optional): _description_. Defaults to Depends(deps.get_current_user).
...
async def create_meta_by_uid(
    ...
    if group in settings.ADMIN_GROUP:
        meta = crud.meta.create(db=db, obj_in=meta_in)
        if not meta:
            raise HTTPException(
                status.HTTP_409_CONFLICT, detail=f"Meta could not be created!"
