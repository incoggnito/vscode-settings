...
from app.schemas.project_user import ProjectUserResponse, ProjectCreateMultipleUsers
...
async def create_project_user(
    workers_in: List[ProjectUserCreate],
    db: Session = Depends(deps.get_db),
    group: str = Depends(deps.get_user_group),
) -> ProjectDef:
    if group in settings.TL_GROUP:
        return [crud.projectuser.create(db, prj_user) for prj_user in workers_in]
