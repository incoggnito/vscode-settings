...
def setup_logging() -> logging.Logger:
    ...
    logfile = (
        timestamp_str + "_Logfile.log"
    )  # TODO include in the name of the logfile the script which is logged,
...
def get_ae_params(config: pd.DataFrame) -> pd.DataFrame:
    ...
    LOGGER.warning(
        " Get_ae_params is not finally implemented. There maybe errors (2 digit after the dezimal point)."
    )
    ...
    samplefrequency = config.loc[1:, "Samplefrequency"].to_numpy(
        dtype=np.int32
    )  # in [Hz]
...
def get_tr_params(config: pd.DataFrame) -> pd.DataFrame:
    ...
    LOGGER.info(
        "Get_tr_params is not finally implemented. There maybe errors (2 digit after the dezimal point)."
    )
...
def hit_from_tra(tra: vae.io.TraRecord) -> vae.io.HitRecord:
    ...
    duration = (
        len(tra.data) / tra.samplerate
    )  # not meaning full if measuring continuosly, in seconds
    energy = vae.features.energy(tra.data, tra.samplerate)
    rms = vae.features.rms(tra.data)
    signal_strength = vae.features.signal_strength(
        tra.data, tra.samplerate
    ...
    if tra.threshold > 0.0:
        ...
        hit = vae.io.HitRecord(
            ...
            energy=energy,
            rms=rms,
            rise_time=vae.features.rise_time(
                tra.data, tra.threshold, tra.samplerate, 0
            ),
            signal_strength=signal_strength,
            trai=trai,
        )
        ...
        def connect_db_inner(
            ...
            try:
                ...
                if not (
                    ...
                    self.db = vae.io._database.Database(
                        str(self.path), mode=mode, table_prefix=self.table_prefix
                    )
                ...
                if self.table_prefix == "tr":
                    ...
                    if "compression" in kwargs:
                        ...
                        self.db = vae.io.TraDatabase(
                            str(self.path), mode=mode, compression=True
                        )
...
def read_acq_settings(db_path: PurePath) -> pd.DataFrame:
    ...
    if "acq_settings" in tables:
        ...
        if not acq_settings.empty:
            ...
            if "index" in acq_settings.columns:
                ...
                acq_settings.loc[1:, "Channel Description"] = acq_settings.loc[
                    1:, "Channel"
                ]
                ...
                if acq_settings.loc[0, "Range"] == "[mV]":
                    acq_settings.loc[1:, "Range"] = 1e-3 * acq_settings.loc[
                        1:, "Range"
                    ].astype(int)
                ...
                LOGGER.warning(
                    "Not support version of the acq_settings in the database."
                )
    ...
    acq_settings.loc[1:, "Range":"Blocksize"] = acq_settings.loc[
        1:, "Range":"Blocksize"
    ].astype(float)
...
def get_first_last_trai(dbfile: "DatabaseFile") -> Tuple[int, int]:
    ...
    """
    trai_start = (
        dbfile.db.connection()
        .execute(f"SELECT min(TRAI) FROM {dbfile.table_prefix}_data;")
        .fetchone()
    )
    ...
    trai_stop = (
        dbfile.db.connection()
        .execute(f"SELECT max(TRAI) FROM {dbfile.table_prefix}_data;")
        .fetchone()
    )
...
def pridb_from_tradb(
    ...
    if tradb_data:
        for ch in range(1, len(tradb_data.channel.unique()) + 1):
            config.loc[ch, "Samplefrequency"] = tradb_data[
                tradb_data.channel == ch
            ].samplerate.unique()[0]
            ...
            config.loc[ch, "Blocksize"] = tradb_data[
                tradb_data.channel == ch
            ].samples.unique()[0]
...
def pridb_live_generation(
    tradbfile_path: PurePath, trai_start: Optional[int] = None
) -> None:
    ...
    with tradb_file(mode="ro") as tradb, pridb_file(mode="rw") as pridb:
        tradb_data = tradb.db.listen(
            existing=True, query_filter=query_filter
        )  # getting a generator
...
def get_new_blocksize(
    tr_data: pd.DataFrame, blocksize: int, fit_size: str = "crop"
) -> pd.DataFrame:
    ...
    for ch, entries in tr_data.groupby("channel"):
        ...
        time = (
            t_start
            + time_step_new
            + np.arange(0, channel_data.shape[0]) * time_step_new
        )
        ...
        df[
            ...
            channel_data.tolist()
        )  # [ ] TODO @AH is there another way than using that tolist?
        df_l.append(df)
...
def modify_raw_data_for_features(
    ...
    if save and tradbfile_path:
        ...
        LOGGER.info(
            f"The names of the files are: {tradbfile_path.stem}{name_extension}"
        )
        pridb_fullpath = pridb_from_tradb(tradbfile_path, tr_new, name_extension)
        trfdb_fullpath = trfdb_from_tradb(
            tradbfile_path, tr_new, name_extension, feature_selection
        )
...
def match_time_to_trai(
    ...
    if time_delta < 0.0:
        LOGGER.info(
            "Timestamp before measurement start. Setting TRAI to first TRAI value."
        )
