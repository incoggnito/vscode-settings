...
from atoolbox import FileHandler, SQLite3Code
...
from bigtoolbox.BigFileHandler.mqtt.publish_data import mqtt_publish_data
...
def get_ae_params(config: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # [ ] Review with the TiePie it is possible to measure with other resolutions than 8-bit ( 8, 12, 14, 16-bits)
    # is the resolution relevant for the ae_parameters?
...
def get_trf_params(config: pd.DataFrame) -> pd.DataFrame:
    # [ ] TODO implement it
...
def hit_from_tra(tra: vae.io.TraRecord, trai: Optional[int] = None) -> vae.io.HitRecord:
    ...
    Args:
        ...
        trai (Optional(int)): trai of the TraRecord. Pass the trai to overwrite/update the existing trai (tra.trai). If no value is passed,
            the current one is used. Defaults to None (using the current one).
...
def read_acq_settings(db_path: PurePath) -> pd.DataFrame:
    ...
    if "acq_settings" in tables:
        ...
        if not acq_settings.empty:
            # following check is needed, to support old acq_settings versions
            expected_columns = [
                "Additional Text",
            ...
            if len(expected_columns) == len(acq_settings.columns) and set(acq_settings.columns) == set(
                expected_columns
            ):
                # newest version of acq_settings (not saving the redundant index in the sql-databases)
                # nothing to transform here
                pass
            elif "index" in acq_settings.columns:
                # medium old acq_settings versions (used from 20/12/2021 to 03/2022)
        ...
        acq_settings = acq_settings.replace(to_replace=["None"], value=np.nan)
        acq_settings = acq_settings.replace(to_replace=["False"], value=False)
        acq_settings = acq_settings.replace(to_replace=["True"], value=True)
        ...
        LOGGER.info(f"\n{acq_settings.loc[1:, float_cols]}")
        LOGGER.info(f"{acq_settings.loc[1:, 'Highpass Filter Cutoff Frequency'].dtypes}")
        acq_settings.loc[1:, float_cols] = acq_settings.loc[1:, float_cols].astype(float)
        acq_settings = acq_settings.replace(to_replace=["None"], value=np.nan)
        ...
        if "tr_data" in tables:
            LOGGER.info("Try to determine the acquisition configuration from the time data.")
            LOGGER.info("These only helps, if you want to create a feature database (.trfdb)")
            LOGGER.info("Not working for creating a tradb or pridb!")
            # [ ] Review with 20 max. lines to read the acq_config of max. 20 channels can be determined
            # should be okay: -> this case is only needed for data before Oct 2021 measured and currently (Feb 2022) max. 6 channels are in the files
            with engine.connect() as conn:
                tr_data_sample = pd.read_sql("SELECT Chan, SampleRate, Samples, Data FROM tr_data LIMIT 20", conn)
            channels = tr_data_sample.Chan.unique()
            blocksize = []
            samplefrequency = []
            for ch in channels:
                samplefrequency.append(tr_data_sample.loc[tr_data_sample["Chan"] == ch, "SampleRate"].iloc[0])
                blocksize.append(tr_data_sample.loc[tr_data_sample["Chan"] == ch, "Samples"].iloc[0])
            acq_settings = pd.DataFrame.from_dict(
                {
                    "Channel": ["[-]"] + channels.tolist(),
                    "Samplefrequency": ["[Hz]"] + samplefrequency,
                    "Blocksize": ["[-]"] + blocksize,
                    "Additional Text": [
                        "[-]",
                        "These acquisition settings are automaticly determined from the tradb data, unknown data like Range, Preamplifergain, Filter settings are omitted.",
                    ]
                    + (len(channels) - 1) * [""],
                }
            )
...
def pridb_from_tradb(
    ...
    Args:
        ...
        name_extension: Optional[str]:  add that string to the original filename (of the tradbfile_path),
            defaults to None
        new_trai: Optional[bool]: reindex the tradb-data (only needed for data with not correct trais)
    ...
    if tradb_data:
        for ch in range(1, len(tradb_data.channel.unique()) + 1):
            config.loc[ch, "Samplefrequency"] = int(tradb_data[tradb_data.channel == ch].samplerate.unique()[0])
            ...
            config.loc[ch, "Blocksize"] = int(tradb_data[tradb_data.channel == ch].samples.unique()[0])
    with pridb_file(mode="rwc") as pridb:
        pridb.create(config)
        LOGGER.info(f"Created empty pridb: {pridb.name}")
        if isinstance(tradb_data, pd.DataFrame):
            if new_trai:
                tradb_data.index = np.arange(1, len(tradb_data) + 1)
            today = datetime.now()
            ...
            with tradb_file() as tradb:
                ...
                try:
                    for i, tra in enumerate(iterable, 1):
                        # tra is a TraRecord!
                        if new_trai:
                            hit = hit_from_tra(tra, i)
                        else:
...
def on_connect(client, userdata, flags, rc):
    LOGGER.info(f"MQTT Client: {client} connected with result code: {rc}")


def on_connect_fail(client, userdata, flags, rc):
    LOGGER.info(f"MQTT Client: {client} connection failed {rc}")


def on_disconnect(client, userdata, rc):
    LOGGER.info(f"MQTT Client {client} disconnected with result code {rc}")


def feature_live_generation(
    tradbfile_path: PurePath,
    ...
    Args:
        tradbfile_path (PurePath): Path to the Tradbfile with the transient data
        trai_start(int):    first TRAI to read, needed if the tradb and pridb is used for previous measurements
        write_trfdb (bool): True: write the features to trfdb, False: do not write the features. Defaults to: True.
        mqtt_publish (bool): True: publish the features via mqtt, False: do not publish. Defaults to: False.
        mqtt_config: (Optional(Dict)): broker-ip and port to use. Defaults to None.

    Returns:
        Tuple(float, int):  last written timepoint, last TRAI
    """
    LOGGER.info("Starting feature live generation")
    wkd = tradbfile_path.parent
    filename = tradbfile_path.name
    f = FileHandler(filename=filename, wkd=wkd)
    if not f.exist_file():
    ...
    if trai_start:
        ...
        query_filter = None
    LOGGER.info(f"MQTT publish option: {mqtt_publish}")
    LOGGER.info(f"MQTT config: \n{mqtt_config}")
    if mqtt_publish:
        if mqtt_config:
            # [ ] TODO go on
            client = paho.Client("VAM_Feature_Publisher")
            ...
            mqtt_publish = False
            LOGGER.info("No MQTT configuration passed. Can not publish the feature data.")
    with tradb_file(mode="ro") as tradb:
        tradb_data = tradb.db.listen(existing=True, query_filter=query_filter)  # getting a generator
        ...
        for tra in tradb_data:
            ...
            if write_trfdb:
                with trfdb_file(mode="rw") as trfdb:
                    trfdb.write(features)
            if mqtt_publish:
                mqtt_publish_data(mqtt_topic, client, features)
