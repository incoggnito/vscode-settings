...
Die Ergebnisse der ersten Messserie zeigen, dass die synthetisch erzeugten Störungen sehr gut auflösbar sind (vgl. \ref{SpekBetriebStoer.png}). Die spektrale Struktur der Phänomene ist  schwierig zu unterscheiden.
...
![Korrelationsmatrix der Features\label{corr.pdf}](images/corr.pdf)
...
Die vorliegenden Messreihen und die skalierten Features lassen sich übergreifend durch eine Reihe von Boxplots vergleichen. Hiermit können Ausreißer und Verteilungen visualisiert werden. In Abbildung  \ref{boxm5hitsnormal.pdf} und \ref{boxm5hitsabnormal.pdf} können Messreihen mit unkritischen Hits mit Störungen verglichen werden.

![Features der Hits im Normalzustand\label{boxm5hitsnormal.pdf}](images/boxm5hitsnormal.pdf){ width=80% }

![Features der Hits als Störung\label{boxm5hitsabnormal.pdf}](images/boxm5hitsabnormal.pdf){ width=80% }

Die folgenden Darstellungen in Abbildung \ref{boxm4timesnormal.pdf} und \ref{boxm4timeabnormal.pdf} stellen die Verteilung des kontinuierlichen Normalzustands und der aufgezeichneten Verschleißgröße dar.

#### Entwicklung einer geeigneten Darstellungsform bei auftretenden Fehlerereignissen

Neben der Datengrundlage und der Architektur für die Zustandsüberwachung wird die Visualisierung im Laufe des Projekts stetig weiterentwickelt. Dabei fließt bei Abstimmungsmeetings und bei den Messterminen das Feedback aller Projektteilnehmer ein. Durch den Einsatz bei den Messungen kann die Praxiseignung aus Sicht der Messingenieure, des Maschinenbedieners und der Softwareentwickler validiert werden. Die Auswahl der Features in Abschnitt \ref{sec:ap6} wird auf Basis der vorangegangen Featureanalyse getroffen.
...
Der Aufbau der verwendeten LSTM-Zelle ermöglicht das Mitführen von Informationen über einen internen Zustand über mehrere Zeitschritte ermöglicht. Damit können dem Modell selbstlernende Eigenschaften zugeschrieben werden. Ein Konzept zur Triggerung eines erneuten Trainings kann die Zustandsüberwachung weiter stabilisieren.
...
Die mathematischen und akustischen Modelle, stellen die gesamte verwendete Pipeline dar. Die Autoencoder sind für die Verschleißerkennung vollständig einseztbar. Die Erkennung der Kerben ist aufgrund fehlender Datenmenge  ...

#### Validierung des Modellansatzes und der Algorithmen

Die Performance des Autoencoder Modells lässt sich durch... validieren

#### Erfassung der Messwerte der unterschiedlichen Einfluss- und Bewertungsgrößen

Im Rahmen der Untersuchungen können unterschiedliche Einfluss- und Bewertungsgrößen ermittelt werden:
...
- Mikroskopische Untersuchung abgeschnittener Drahtstücke

Es lässt sich zeigen, dass die Ziehgeschwindigkeit nicht proportional von der gemessenen Körperschallamplitude abhängt.
...
**Meilenstein 4:** Die Demonstration einer ersten Fehlerdiagnostik und innovativer Algorithmen kann ohne neuentwickelte Sensoren, daher nur teilweise umgesetzt werden.
...
Für die Gestaltung der einzelnen Registerkarten werden möglichst gleiche Elemente, Schriftarten und Farben gewählt. Um den Prozesszustand stets überwachen zu können, werden die dazugehörigen Elemente auf jeder Registerkarte an der gleichen Position im Fenster positioniert.

**Kennzeichnung wichtiger Elemente über Textur oder Farbe**

Farblich herausstechend sind im Wesentlichen die Signale der Sensoren und das Ergebniss der Prozessüberwachung. Die farbliche Gestaltung und somit Beurteilung des aktuellen Prozesszustandes basiert auf den Ampelfarben (Grün, Gelb oder Orange?, Rot). So kann der Bediener stets erkennen, ob ein Eingreifen erforderlich ist und wie schnell gehandelt werden muss. Weitere Farben finden sich in der Visualisierung der Messdaten, wie bspw. in der Registerkarte „Spektrogramm“ (vgl. \ref{SpekLive.pdf}) wieder.
