...
from measurementgui.ui_files.LaserDetectionGUI import Ui_MainWindow

# from ui_files.CMS_GUI import Ui_MainWindow
from bigtoolbox import FileHandler
...
from pytiepiesql.deviceinterface import tiepie_controler, check_connection
from pytiepiesql.measurement import trfdb_writer
import logging
...
class CalibrationDialog(QDialog):
    def __init__(self, parent: QMainWindow, windowtitle: str, message: str):
        # [ ] Review variable type is not that specific, but MainWindow is declared later and it is not possible to declare SaveDialog later
        super().__init__()

        self.setWindowTitle(windowtitle)

        self.button_select = QPushButton("Eine Messung auswaehlen")
        self.button_record = QPushButton("Neue Messung aufnehmen")

        buttonLayout = QHBoxLayout()
        buttonLayout.addWidget(self.button_select)
        buttonLayout.addWidget(self.button_record)
        # lambda function is needed to pass the SaveDialog object to close it in get_file_location
        self.button_select.pressed.connect(
            lambda: parent.get_file_location(self, mode="open", select_calibration=True)
        )
        self.button_record.pressed.connect(lambda: parent.automatic_calibration(self))
        # [ ] TODO go on with the calibration menu
        # -> also in the qtdesigner -> diff of laser and normal
        # [ ] TODO replace the features?
        self.layout = QVBoxLayout()
        message = QLabel(message)
        self.layout.addWidget(message)
        self.layout.addLayout(buttonLayout)
...
class CalibrationDialog_further(QDialog):
    def __init__(self, parent: QMainWindow, windowtitle: str, message: str):
        # [ ] Review variable type is not that specific, but MainWindow is declared later and it is not possible to declare SaveDialog later
        super().__init__()
        ...
        self.button_select = QPushButton("Weitere Messung auswaehlen")
        self.button_done = QPushButton("Fertig")

        buttonLayout = QHBoxLayout()
        buttonLayout.addWidget(self.button_select)
        buttonLayout.addWidget(self.button_done)
        # lambda function is needed to pass the SaveDialog object to close it in get_file_location
        self.button_select.pressed.connect(lambda: parent.start_calibration_dialog(again=True, dialog=self))
        self.button_done.pressed.connect(
            lambda: parent.evaluate_calibration_measurements(add_calibration=False, dialog=self)
        )
        # [ ] TODO go on with the calibration menu
        # -> also in the qtdesigner -> diff of laser and normal
        # [ ] TODO replace the features?
...
class MeasurementInterface(QMainWindow):
    def __init__(self):
        ...
        # self.p_pridb = Noneecho $projs | sed 's/ /\n /g' | xargs -I{} bash -c "echo start {}; git -C {} fetch; git -C {} checkout dev; cd {}; activate; poetry install; cd ..; echo end {}"
        ...
        self.ui.calibration_normal.pressed.connect(self.start_calibration_dialog)
        self.ui.calibration_laser.pressed.connect(self.get_laser_record_and_evaluate)
    ...
    # def try_connect(self, error_msgs=True) -> None:
    # [ ] TODO nice try, but with this try_connect and successfull conection to an devices the measurment fails always
    #      """Repeatedly tries to detect connected devices

    #     Args:
    #         error_msgs(bool): display error messages. Defaults to true."""
    #     self.connected_devices = check_connection(error_msgs)
    #     self.reconnect_Timer = QtCore.QTimer()
    #     self.reconnect_Timer.timeout.connect(lambda: self.try_connect(error_msgs=False))
    #     if not len(self.connected_devices) > 0:
    #         # [ ] REVIEW 3 connection tries per second^
    #         if error_msgs:
    #             LOGGER.info("Going to try to detect connected devices continuously.")
    #         self.reconnect_Timer.start(300)
    #     else:
    #         self.reconnect_Timer.stop()
    #         del (
    #             self.connected_devices
    #         )  # [ ] REVIEW is it necessary to delete one connection to open later one a connection to measure??
    ...
    def start_measurement(self):
        ...
        if self.outputfile_path:
            ...
            if not self.p_meas or self.p_meas.state() == QtCore.QProcess.NotRunning:  # no process running
                ...
                measurementinterface_script = PurePath(inspect.getsourcefile(tiepie_controler))
                self.p_meas.start(
                    str(PYTHON_VENV),
                    [
    ...
    # def process_finieshed(self, msg: str, process: str) -> None:
    #     """Logs that the process is finished and removes the process id

    #     Args:
    #         msg (str): log message
    #         process (str): process to remove
    #     """
    #     self.message_and_log(msg)
    #     if process == "meas":
    #         self.p_meas = None
    #     elif process == "trfdb":
    #         self.p_trfdb = None
    ...
    def get_file_location(
        self,
        filedialog: Optional[Union[FileDialog, CalibrationDialog]] = None,
        mode: str = "save",
        attempt: int = 0,
        select_calibration: bool = False,
    ) -> None:
        ...
        if mode == "save":
            ...
            name = QFileDialog.getSaveFileName(
                self,
                "Kalibriermessungen abspeichern",
        ...
        if name[0]:
            ...
            self.set_filehandler(select_calibration)
            self.trfdbfile = PurePath(self.outputfile_path.parent, self.outputfile_path.stem + ".trfdb")
            ...
            if attempt < 4:
                ...
                # [x] TODO break here? Okay like this

    def set_filehandler(self, select_calibration: bool = False) -> None:
        if self.outputfile_path:
    ...
    def get_alarm_thresholds(self) -> None:
        ...
        self.alarm_thresholds[self.ui.label_29.text()] = pd.Series(
            [
                ...
                self.ui.doubleSpinBox_10.value(),
                self.ui.doubleSpinBox_11.value(),
                self.ui.doubleSpinBox_12.value(),
            ]
        )
        ...
        self.alarm_thresholds[self.ui.label_28.text()] = pd.Series(
            [
                ...
                self.ui.doubleSpinBox_15.value(),
                self.ui.doubleSpinBox_16.value(),
                self.ui.doubleSpinBox_17.value(),
                self.ui.doubleSpinBox_18.value(),
            ]
        )
        ...
        self.alarm_thresholds[self.ui.label_38.text()] = pd.Series(
            [
                ...
                self.ui.doubleSpinBox_21.value(),
                self.ui.doubleSpinBox_22.value(),
                self.ui.doubleSpinBox_23.value(),
                self.ui.doubleSpinBox_24.value(),
            ]
        )
    ...
    def display_alarm_thresholds(self) -> None:
        ...
        alarm_thresh_spin_boxes = [
            ...
            self.ui.doubleSpinBox_11,
            self.ui.doubleSpinBox_12,
        ]
        ...
        alarm_thresh_spin_boxes = [
            ...
            self.ui.doubleSpinBox_15,
            self.ui.doubleSpinBox_16,
            self.ui.doubleSpinBox_17,
            self.ui.doubleSpinBox_18,
        ]
        ...
        alarm_thresh_spin_boxes = [
            ...
            self.ui.doubleSpinBox_21,
            self.ui.doubleSpinBox_22,
            self.ui.doubleSpinBox_23,
            self.ui.doubleSpinBox_24,
        ]
            ...
            if not temp_data.empty:
                ...
                for ch, ch_data in cur_data.groupby("Channel"):
                    ...
                    if alarm_result.any():
                        ...
                        # [x] TODO set the Channel to green after time interval again? User has a button to set them green again
    ...
    def change_channel_background(self, channel: int, color: str) -> None:
        ...
        if channel == 1:
            ...
            self.ui.label_24.setStyleSheet(f"background-color: {color}")  # Sensor 3
        elif channel == 4:
            self.ui.label_23.setStyleSheet(f"background-color: {color}")  # Sensor 4
        elif channel == 5:
            self.ui.label_22.setStyleSheet(f"background-color: {color}")  # Sensor 5
        elif channel == 6:
            self.ui.label_21.setStyleSheet(f"background-color: {color}")  # Sensor 6
