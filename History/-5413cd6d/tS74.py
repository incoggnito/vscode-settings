...
class MeasurementInterface(QMainWindow):
    ...
    # @qasync.asyncSlot()
    def init_mqtt_gui(self):
        self.message_and_log("Starting the mqtt subscriber.")
        # [ ] TODO add on message function
        self.client = init_mqtt(MQTT_CONFIG, "MeasurementGUI_Subscriber")
    ...
    def plot_featuredata(self, index: int) -> None:
        if not self.replay in ["pause", "stopp"]:
            if not MQTT and self.trfdbfile:
                if not self.trfdb:
