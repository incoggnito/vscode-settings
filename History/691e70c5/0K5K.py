...
def test_yield_batches() -> None:
    """Test the parquetfile handling"""
    f = FileHandler(
        filename="20220118_Brute Force Match_Labled.parquet",
        wkd=r"C:\Users\KBenkler\Documents\Forschungsprojekte\00_Aktiv\Panamera\Messdaten\22-01-20_Auswertung_Melvin",
    )
    iter = f.current_file.yield_batches(use_pandas_metadata=True)
    for row in iter:
        df_temp = row.to_pandas()
        pass
    assert False
