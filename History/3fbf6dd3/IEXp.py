import logging
import os
from datetime import datetime, date, combine, time
...
class Timesheet:
    ...
    def __post_init__(self):
        super().__init__(self)
        self.begin = self._check_begin_date(self.begin)
        self.end = self._checkdate(self.end)

    @staticmethod
    def _check_begin_date(dt: Union[date, datetime, str]) -> str:
        if isinstance(dt, datetime):
            dt = dt
        elif isinstance(dt, date):
            date = combine(date, time(0, 0))
