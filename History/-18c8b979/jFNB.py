...
def oauth_token(authorization=Header(None), token=Depends(oauth2_scheme)):
    if not authorization:
        return None
    return token


def api_key(authorization=Header(None), api_key=Depends(api_scheme)):
    if not authorization:
        return None
...
async def get_current_user(
    ...
    try:
        if access_type == "api_key":
            user = crud.user.get_by_uid(db, uid=1000)  # TODO get user defined by token
            if user:
                return user
            else:
                raise credentials_exception
