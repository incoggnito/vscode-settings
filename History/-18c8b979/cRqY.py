...
from app.core.auth import TokenVerifier, OAuth2TokenBearer
from app.db.session import SessionLocal
from app.models.user import User
from app import crud
from app.core.config import settings
from app.core.auth import APIKeyHeader
...
def api_key():  # TODO simple API Key auth
    try:
        return APIKeyHeader(name="Authorization")
    except:
        raise Exception


# https://stackoverflow.com/questions/64731890/fastapi-supporting-multiple-authentication-dependencies
def authify(phantom_token: str = Depends(api_key), token: str = Depends(oauth_token)):
    if not (phantom_token or token):
        raise Exception
    if token:
        return token
    else:
