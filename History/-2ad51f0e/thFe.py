...
from atoolbox.FileHandler import FileHandler
...
from fyrsonic.measurement.outputfolder import create_output_folder_structure
...
def classify(rms: float, hfc: float) -> Label:
    """Inference based on feature vector, just a placeholder for your logic/model."""
    if rms >= 10e-6 and hfc > 1e-7:  # >= 1 mV
        return Label(label="Kerbe")
    return Label(label="Keine Kerbe")
...
async def read_write(
    ...
    await write_features(
        queue, data, stream_config, trfdbfile, trai_last, feature_selection
    )
...
async def write_features(
    ...
    param_id = 3  #: Parameter ID of table ae_params for ADC value conversion
    for ch in data.keys():
        ...
        with ThreadPoolExecutor() as pool:
            ...
            label = await loop.run_in_executor(
                pool, classify, features["RMS"], features["HFC"]
            )
        queue.put_nowait((ch, time, features["RMS"], features["Crest_Factor"], label))
