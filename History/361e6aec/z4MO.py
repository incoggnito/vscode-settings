...
from fyrsonic.mqtt_pub.common import publish_dataframe, BROKER, set_parameter
...
async def cmd_input():
    ...
    await publish_dataframe(broker=BROKER, topic="user_meta", df=df2)

    while True:
        df = await set_parameter(variables, df)
        df["Time"] = pd.Timestamp.now()
        #df.to_sql(name="tblUser", con=engine, if_exists="append", index=False)
        await publish_dataframe(broker=BROKER, topic="user_data", df=df)
